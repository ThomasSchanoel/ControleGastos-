...
$sql_saude = "SELECT * FROM bia_compras WHERE categoria = 'saude' ORDER BY data ASC";
$query_saude = $mysqli->query($sql_saude) or die($mysqli->error);
$num_compras = $query_saude->num_rows; 
...
<body>
    <header>
        <h1>Saúde - R$<?php echo number_format($saude, 2, ',', '.'); ?></h1>
    ...
    <main>
        ...
        <div class="container">
            ...
            <?php else: ?>
                <?php $query_saude->data_seek(0); ?>
                <?php while ($compra = $query_saude->fetch_assoc()): 
